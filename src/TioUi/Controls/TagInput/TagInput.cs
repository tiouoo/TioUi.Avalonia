...
public class TagInput : TemplatedControl
{
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
    {
        ...
        {
            var oldTags = change.GetOldValue<IList<string>?>();
            if (oldTags is INotifyCollectionChanged oldObservable)
            {
                oldObservable.CollectionChanged -= OnTagsCollectionChanged;
            }

            if (Tags is INotifyCollectionChanged newObservable)
            {
                newObservable.CollectionChanged += OnTagsCollectionChanged;
            }
            CheckEmpty();
        }
    }
    ...
    protected override void OnLoaded(RoutedEventArgs e)
    {
        ...
        ObservableExtension.Subscribe(InputTextBox.GetObservable(TextBox.TextProperty), _ => CheckEmpty());

        _presenter = InputTextBox.GetTemplateDescendants().OfType<TextPresenter>().FirstOrDefault();
        if (_presenter is not null)
            ObservableExtension.Subscribe(_presenter.GetObservable(TextPresenter.PreeditTextProperty), _ => CheckEmpty());
    }
